...
1. Create a `multi_agent_pirate.yaml` file with the following contents:

    ```yaml save-as=multi_agent_pirate.yaml
    version: "2"
    ...
    > [!NOTE]
    > The `pirate` agent has a `description`, this is required when an agent is
...
1. Create this advanced multi-agent system by creating a `dev_team.yaml` file with the following contents:

    ```yaml save-as=dev_team.yaml
    version: "2"
...
3. Try asking it to build a simple web application, like: 
    ...
    ```console
    Build a simple todo app with a nice UI
    ```
